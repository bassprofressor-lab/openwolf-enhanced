// ─────────────────────────────────────────────────────────────────────────────
// Symbol-level anatomy (ported from upstream cytostack/openwolf 2.0, §F2b).
//
// Extracts top-level symbols (name, kind, line range, token estimate) so a
// pre-read hint can point the agent at a slice of a big file — read one
// function with offset/limit instead of the whole file. Deliberately heuristic:
// line-anchored regexes for top-level declarations only; a symbol's end is the
// line before the next symbol. Crude ranges are fine for offset/limit reads.
//
// Self-contained (no relative imports) so tests can import it directly and the
// scanner can pull it without dragging in the anatomy store.
// ─────────────────────────────────────────────────────────────────────────────

export interface SymbolEntry {
  name: string;
  kind: "fn" | "class" | "section";
  startLine: number;
  endLine: number;
  tokens: number;
}
...
export function symbolsSupported(ext: string): boolean {
...
export function extractSymbols(content: string, ext: string): SymbolEntry[] {
  ...
  if (!patterns || content.length > SYMBOL_MAX_BYTES) return [];

  const lines = content.split(/\r?\n/);
  const found: Array<{ name: string; kind: SymbolEntry["kind"]; startLine: number }> = [];
